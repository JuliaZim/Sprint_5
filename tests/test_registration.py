...
class TestRegistration:
    ...
    def test_registration_success(self, browser, registration_popup):
        ...
        driver.find_element(*RegistrationPageLocators.SUBMIT_PASSWORD_INPUT).send_keys(
            password
        )

        # Кликаем кнопку Создать аккаунт
        driver.find_element(*RegistrationPageLocators.CREATE_ACCOUT_BUTTON).click()

        # Проверка, что произошел вход, есть фото профиля и имя USER
        wait.until(EC.presence_of_element_located(MainPageLocators.USER))
        assert (
            driver.find_element(*MainPageLocators.CREATE_AD_BUTTON).is_displayed()
            and driver.find_element(*MainPageLocators.USER).text == "User."
            and driver.find_element(*MainPageLocators.AVATAR).is_displayed()
        )
    ...
    def test_registration_with_incorrect_email(self, registration_popup):
        ...
        try:
            error_email = wait.until(EC.visibility_of_element_located(RegistrationPageLocators.ERROR_EMAIL))
            error_password_parent = wait.until(EC.visibility_of_element_located(RegistrationPageLocators.ERROR_PASSWORD))
            error_submit_password_parent = wait.until(EC.visibility_of_element_located(RegistrationPageLocators.ERROR_SUBMIT_PASSWORD))
            error_email_text = wait.until(EC.visibility_of_element_located(RegistrationPageLocators.ERROR_TEXT_EMAIL))
